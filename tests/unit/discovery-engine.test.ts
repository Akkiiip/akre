...
const base = (id: string): SourceObservation => ({
  id,
  workspaceId: "default",
  mode: "LIVE",
...
it("discovery rejects duplicate provider evidence and preserves no partial data on failure", async () => {
  ...
  const job = service.base("job") as never;
  const provider: DiscoveryProvider = {
    id: "test-source",
    evidenceKinds: ["ATTENTION"],
    validate: (v) => v as Record<string, unknown>,
    fetch: async () => [base("a"), base("a")],
    extractSignals: () => [],
  };
  await expect(runDiscovery(service, job, provider)).rejects.toThrow("duplicate");
