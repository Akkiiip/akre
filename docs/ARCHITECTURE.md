...
```
React / Vite → same-origin Express API → Service → SQLite repository
...
The first backend is Node 24 + Express + Node SQLite. It is intentionally a **single-owner, single-workspace, single-process deployment foundation**, not completed multi-tenant SaaS. SQLite stores typed JSON records with workspace isolation, a workspace foreign key, transactional writes, indexed record kinds and a migration ledger. Domain references are validated at service boundaries. This keeps economic and decision snapshots intact while the model evolves. The current API returns the full workspace; pagination is a required next step before large ingestion volumes.
