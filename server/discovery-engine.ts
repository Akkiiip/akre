...
export type DiscoveryEvidenceKind = "ATTENTION" | "DEMAND" | "PURCHASE";
...
export async function runDiscovery<TInput>(
  ...
  service.repo.transaction(() => {
    ...
    for (const row of rows) {
      ...
      const prior = service.repo.list("observations").find((item) => item.sourceId === row.sourceId && item.externalId === row.externalId);
      if (prior) {
        if (JSON.stringify(prior.payload) !== JSON.stringify(row.payload))
          throw new Error("Provider revised stored evidence; explicit reconciliation is required");
      } else {
    ...
    for (const productId of productIds) {
      ...
      const evidence = service.repo.list("observations").filter((item) => item.sourceId === provider.id && ((product.sourceReferences ?? []).includes(item.reference ?? "") || item.productName === product.name));
      for (const signal of provider.extractSignals(evidence, productId))
        if (!service.repo.list("signals").some((old) => old.id === signal.id)) service.repo.put("signals", signal);
      recompute(service, productId, "DISCOVERY_RUN");
    }
    service.audit("DISCOVERY_RUN_COMPLETED", job.id, {
      provider: provider.id,
      evidenceKind: provider.evidenceKind,
      received: rows.length,
      inserted,
      productIds: [...productIds],
      meaning: provider.evidenceKind === "ATTENTION" ? "Attention evidence only; not purchase demand or purchase proof." : `${provider.evidenceKind} evidence as declared by the provider.`,
    });
  });
}