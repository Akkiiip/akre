...
const NON_PRODUCT_PATTERNS = [
  ...
  /\b(news|headline|breaking|politics|politician|prime minister|president|celebrity|actor|actress|movie|film|trailer|episode|series|serial)\b/i,
  /\b(stock market|share price|bitcoin price|gold price|sensex|nifty)\b/i,
];

const PRODUCT_RULES: Array<[string, RegExp]> = [
  ["beauty", /\b(serum|sunscreen|lipstick|makeup|concealer|moisturizer|shampoo|conditioner|skincare|hair oil|trimmer|straightener)\b/i],
  ["home", /\b(organizer|storage|cushion|curtain|lamp|light|cleaning|mop|hanger|decor|pillow|bedsheet)\b/i],
  ["kitchen", /\b(chopper|bottle|mixer|grinder|air fryer|fryer|container|lunch box|kitchen|cookware|peeler|grater)\b/i],
  ["fashion", /\b(dress|shirt|tshirt|t-shirt|kurti|saree|jeans|jacket|hoodie|shoes|sandal|slipper|bag|wallet)\b/i],
  ["accessories", /\b(jewellery|jewelry|earrings|necklace|bracelet|ring|watch|sunglasses|case|cover)\b/i],
  ["gadgets", /\b(earbuds|headphones|speaker|charger|power bank|smartwatch|keyboard|mouse|camera|projector|gadget|usb)\b/i],
  ["fitness", /\b(dumbbell|resistance band|yoga|fitness|gym|massager|workout)\b/i],
  ["pets", /\b(pet|dog|cat|leash|collar|toy for dogs|toy for cats|pet bed)\b/i],
  ["automotive", /\b(car|bike|motorcycle|dashboard|car mount|car cover|bike cover|phone holder)\b/i],
  ["kids", /\b(toy|kids|baby|infant|school bag|puzzle|stroller)\b/i],
];
...
export function classifyProductQuery(query: string): { classification: ProductClassification; category: string; reason: string } {
  ...
  if (/\b(buy|shop|price|deal|offer|best|top|cheap|under \d+|amazon|flipkart)\b/i.test(normalized)) {
